...
def save_candle(candle: dict):
    conn = sqlite3.connect(DB_PATH)
    try:
        conn.execute('INSERT OR REPLACE INTO candles (timestamp,open,high,low,close,volume) VALUES (?,?,?,?,?,?)',
...
def load_candles(limit: int = HISTORY_LIMIT) -> List[dict]:
    conn = sqlite3.connect(DB_PATH)
    rows = conn.execute('SELECT timestamp,open,high,low,close,volume FROM candles ORDER BY timestamp DESC LIMIT ?', (limit,)).fetchall()
    conn.close()
    return [{"ts": r[0], "open": r[1], "high": r[2], "low": r[3], "close": r[4], "volume": r[5]} for r in reversed(rows)]

def save_prediction(p: dict):
    conn = sqlite3.connect(DB_PATH)
    try:
        conn.execute('INSERT INTO predictions (window_start,window_end,predicted_signal,confidence,actual_signal,actual_open,actual_close,result) VALUES (?,?,?,?,?,?,?,?)',
                     (p["window_start"], p["window_end"], p["predicted"], p["confidence"],
                      p.get("actual", "⏳"), p.get("act_open", 0), p.get("act_close", 0), p.get("result", "⏳")))
        conn.commit()
    except Exception as e:
...
def update_prediction_result(ws, we, actual, ao, ac, result):
    conn = sqlite3.connect(DB_PATH)
    try:
        conn.execute('UPDATE predictions SET actual_signal=?,actual_open=?,actual_close=?,result=? WHERE window_start=? AND window_end=?',
...
def load_predictions(limit: int = 10) -> List[dict]:
    conn = sqlite3.connect(DB_PATH)
    rows = conn.execute('SELECT window_start,window_end,predicted_signal,confidence,actual_signal,actual_open,actual_close,result FROM predictions ORDER BY id DESC LIMIT ?', (limit,)).fetchall()
    conn.close()
    return [{"window_start": r[0], "window_end": r[1], "window": f"{r[0]}-{r[1]}", "predicted": r[2], "confidence": r[3],
             "actual": r[4] if r[4] else "⏳", "act_open": r[5] or 0, "act_close": r[6] or 0, "result": r[7] if r[7] else "⏳"} for r in rows]

def save_model(obj, wins: int, losses: int):
    conn = sqlite3.connect(DB_PATH)
    try:
        blob = pickle.dumps(obj)
        conn.execute('INSERT OR REPLACE INTO model (id,model_data,wins,losses) VALUES (1,?,?,?)', (blob, wins, losses))
        conn.commit()
...
def generate_synthetic_history(n: int = 80) -> List[dict]:
    ...
    for i in range(n):
        ...
        candles.append({"ts": t + i * WINDOW_SECONDS, "open": round(o, 2), "high": round(h, 2),
                        "low": round(lo, 2), "close": round(c, 2), "volume": round(random.uniform(5, 50), 4)})
        price = c
    return candles
...
class AdaptiveEnsemble:
    def __init__(self):
        ...
        self.rf_clf = None
        self.lgb_clf = None
        self.result_history = [[] for _ in range(5)]
        self.weights = list(BASE_WEIGHTS)

    def _model_available(self) -> List[bool]:
        return [self.bilstm is not None, self.attn_gru is not None, self.xgb_clf is not None,
                self.rf_clf is not None, self.lgb_clf is not None]

    def update_adaptive_weights(self):
        avail = self._model_available()
        min_history = min((len(self.result_history[i]) for i, a in enumerate(avail) if a), default=0)
        ...
        logger.info(f"Ensemble weights → BiLSTM:{self.weights[0]:.2f} AttnGRU:{self.weights[1]:.2f} XGB:{self.weights[2]:.2f} RF:{self.weights[3]:.2f} LGB:{self.weights[4]:.2f}")

    def record_result(self, per_model_preds: List[Optional[int]], actual_class: int):
    ...
    def predict_proba(self, X_seq: np.ndarray, X_flat: np.ndarray) -> Tuple[np.ndarray, List[Optional[int]]]:
        probs, used_w, per_model_preds = [], [], [None]*5
        if self.bilstm:
            p = _torch_infer(self.bilstm, X_seq)
            if p is not None:
                probs.append(p); used_w.append(self.weights[0]); per_model_preds[0] = int(np.argmax(p[0]))
        if self.attn_gru:
            p = _torch_infer(self.attn_gru, X_seq)
            if p is not None:
                probs.append(p); used_w.append(self.weights[1]); per_model_preds[1] = int(np.argmax(p[0]))
        if self.xgb_clf:
            try:
                p = self.xgb_clf.predict_proba(X_flat)
                probs.append(p); used_w.append(self.weights[2]); per_model_preds[2] = int(np.argmax(p[0]))
            except Exception as e:
                logger.error(f"XGB infer: {e}")
        if self.rf_clf:
            try:
                p = self.rf_clf.predict_proba(X_flat)
                probs.append(p); used_w.append(self.weights[3]); per_model_preds[3] = int(np.argmax(p[0]))
            except Exception as e:
                logger.error(f"RF infer: {e}")
        if self.lgb_clf:
            try:
                p = self.lgb_clf.predict_proba(X_flat)
                probs.append(p); used_w.append(self.weights[4]); per_model_preds[4] = int(np.argmax(p[0]))
            except Exception as e:
...
def _seed_pending_row_locked(window_start_ts: float) -> None:
    ...
    seen = set()
    for row in history_rows:
        if row["window_start"] not in seen:
            unique.append(row)
...
def _close_current_window_locked() -> None:
    ...
    for idx, row in enumerate(history_rows):
        if row.get("actual") == "⏳" and row.get("window_start") == ws_str:
            ...
            if predicted == "HOLD":
                row["result"] = "—"
                update_prediction_result(ws_str, we_str, actual, candle["open"], candle["close"], "—")
                ...
                if predicted == actual:
                    ...
                    row["result"] = "❌"
                    losses += 1
                update_prediction_result(ws_str, we_str, actual, candle["open"], candle["close"], row["result"])
                ...
                logger.info(f"Window {ws_str}-{we_str}: Pred={predicted} Act={actual} {row['result']}")
            break
    else:
    ...
    while True:
        try:
            def on_open(w):
                nonlocal retry
                retry = 2
                w.send(json.dumps({"type": "subscribe", "product_ids": ["BTC-USD"], "channels": ["ticker", "matches"]}))
            ...
            def on_message(w, raw):
                try:
                    ...
                    pass
            ws_client.WebSocketApp(url, on_open=on_open, on_message=on_message,
                                   on_error=lambda w, e: logger.error(f"WS err: {e}"),
                                   on_close=lambda w, c, m: logger.warning("WS closed")).run_forever(ping_interval=20, ping_timeout=10)
        except Exception as e:
...
def _build_state_payload() -> dict:
    ...
    return {"price": round(live_price, 2), "signal": pred["signal"], "confidence": pred["confidence"],
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: #080C14; color: #C8D8EF; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', sans-serif; min-height: 100vh; }
  ...
  .status-bar { display: flex; align-items: center; gap: 10px; margin-bottom: 12px; font-size: 0.8rem; color: #4A6080; flex-wrap: wrap; }
...
async def lifespan(application: FastAPI):
    ...
    with state_lock:
        ...
        if not exists and len(completed_candles) > 0:
            _seed_pending_row_locked(current_ws)
